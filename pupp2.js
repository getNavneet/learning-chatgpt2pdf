...
(async () => {
  ...
  // Original HTML content
    const htmlContent = `<div class="overflow-y-auto p-4" dir="ltr">
...
`;
  ...
  // Extract and process code lines
  const codeBlocks = await page.evaluate(() => {
    const container = document.querySelector('.language-js');
    const lines = [];
    let currentLine = [];
  ...
  // Generate formatted HTML with proper line separation
  const formattedHtml = `
  <html>
    <head>
      <style>
  ...
  await pdfPage.setContent(formattedHtml);
  
  await pdfPage.pdf({
    path: 'properly-separated-code.pdf',
    format: 'A4',
  ...
  await browser.close();
  console.log('PDF with properly separated lines created!');
})();